...
# Convert defined columns to proper date and time formats.
for i in autos.loc[:,'date_crawled']:
    date_crawled = i
    date_crawled = dt.datetime.strptime(date_crawled, date_format)
    i = date_crawled 
    
for i in autos.loc[:,'ad_created']:
    ad_created = i
    ad_created = dt.datetime.strptime(ad_created, date_format)
    i = ad_created
    
for i in autos.loc[:,'last_seen']:
    last_seen = i
    last_seen = dt.datetime.strptime(last_seen, date_format)
    i = last_seen
    
for i in autos.loc[:,'last_seen_hour']:
    last_seen_hr = i
    last_seen_hr = dt.datetime.strptime(last_seen_hr, time_format)
    i = last_seen_hr
...
# Remove all rows where the vehicle type is a null value.
autos = autos.drop(autos[autos["vehicle_type"] == 'nan'].index)

# Remove all rows where the model is an integer.
autos = autos.drop(autos[(autos["model"] == '80') | (autos["model"] == '147') | (autos["model"] == '159') | (autos["model"] == '911') | (autos["model"] == '90') | (autos["model"] == '500') | (autos["model"] == '9000') | (autos["model"] == '200') | (autos["model"] == '145') | (autos["model"] == '156')].index)

# Convert to a CSV file.
autos.to_csv('autos_clean.csv', index = True)